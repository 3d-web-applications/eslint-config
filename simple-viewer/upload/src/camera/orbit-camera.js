...
Object.defineProperty(OrbitCamera.prototype, 'pivotPoint', {
  pivotPoint: () => this._pivotPoint,
  pivotPoint: (value) => this._pivotPoint.copy(value),
});
...
OrbitCamera.prototype = {
  ...
  update: (dt) => {
    // Add inertia, if any
    const t = this.inertiaFactor === 0
      ? 1 : Math.min(dt / this.inertiaFactor, 1);
  ...
  _clampPitchAngle: (pitch) => {
    const result = pc.math.clamp(
      pitch, -this.pitchAngleMax, -this.pitchAngleMin,
    );
